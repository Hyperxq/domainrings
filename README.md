...
2. Use **Example** in the toolbar to load another one:
   ...
   - **Two slices, one link (preview)**: two hexagons joined by one link, an early look at the context map (see [What is coming](#what-is-coming)).
3. Try three things:
...
4. Keep your work: **Export → .hexa** saves the model as a file you can open again later with **Import**. **Export → SVG** or **PNG** gives you an image for documents and slides.
...
Your work autosaves in this browser's `localStorage` a moment after each change, so a reload brings it back. Export a `.hexa` file for anything you want to keep or share, and open it again with **Import**.
...
The context map is in progress: several hexagons placed side by side as a honeycomb, adding a hexagon from the canvas, bringing a hexagon into a bounded context, and links between hexagons routed through their ports and labelled with the DDD relationship they represent (anticorruption layer, open host service, customer-supplier, conformist, shared kernel). The file format already has room for it. The **Two slices, one link (preview)** example is the way to see it today.
